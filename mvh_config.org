...
#+begin_src emacs-lisp :var agenda-heading="Agenda" :var agenda-heading-depth=1 :var include-depth=1 :results value raw :exports none
  ...
  ,#+REVEAL_TOC: headlines "
            (format "%s\n" include-depth)
            "#+LATEX: \\renewcommand{\\printtoctitle}[C]{}\n"
...
# C. Unique grid number (e.g., starting from one)
# algorithms. File with names of images to position on the grid
# memory. Height of the grid as percentage of the height of the viewport
#    (used with CSS unit vh)
# data_structures/html_css_js. Numbers of (a) columns and (b) rows of the grid
# python. Grid-template-areas to specify what image to position on what
#    grid cell(s); see, e.g.:
#    https://developer.mozilla.org/en-US/docs/Web/CSS/grid-template-areas
#    With this macro, the grid areas *must* be named ga1, ga2, ...
# web_programming. Optional indicator whether the grid as a whole (value grid) or individual
